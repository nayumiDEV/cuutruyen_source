<template>
  <div class="mx-2 py-8 lg:py-16">
    ...
    <CuuPaginate routeName="TeamProfileMangasNew" :currentPage="currentPage" :totalPages="totalPages" />
  </div>
</template>
...
export default {
  ...
  metaInfo () {
    return {
      title: `Mới cập nhật - ${this.team.name}`
    }
  },
  ...
  async mounted () {
    ...
    this.$gtag.pageview({ page_title: `Mới cập nhật - ${this.team.name}` })
  ...
  methods: {
    async loadMangas (page) {
      this.loading = true
      try {
        const response = await getTeamMangasNew(this.team.id, page, 30)
