...
impl<T: rand::Rng + rand::RngCore> RngCaptcha<T> {
    ...
    pub fn add_custom_char(&mut self, c: char) -> &mut Self {
        let cimg = match self.font.png(c) {
            None => None,
            Some(p) => Image::from_png(p).map(|i| (c, i)),
        };

        if let Some((c, i)) = cimg {
            let x = self.text_area.right;
            let y = (self.text_area.bottom + self.text_area.top) / 2 - i.height() / 2;
            self.img.add_image(x, y, &i);
    ...
    pub fn add_custom_chars(&mut self, chars: Vec<char>) -> &mut Self {
        for c in chars {
            self.add_custom_char(c);
        }
        self
    }
